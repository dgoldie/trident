defmodule Gateway.Play.Response do
  @moduledoc false
  ...
  @spec play?() :: boolean
  def play?(), do: Application.get_env :gateway, :play, false
  ...
  defp gen_response() do
    GatewayFile.get_mapping_path
    |> GatewayFile.json_files!
    |> Enum.reduce([], fn path, acc ->
      json = validate(GatewayFile.read_json_file!(path))
      key = gen_key(json)
  ...
  ## Example
      iex> Gateway.Play.Response.play_paths("path")
      ["/request/path", "/request/path"]
      iex> Gateway.Play.Response.play_paths("path_pattern")
      ["\\A/request.*neko\\z"]
      iex> Gateway.Play.Response.play_paths("no_pattern")
      []
  """
