defmodule Gateway.Handle do
  ...
  alias Gateway.Play.Data
  alias Gateway.Record.Response, as: Record
  alias Gateway.Play.Response, as: Play
  alias Gateway.Play.Paths, as: PlayPaths
  alias Gateway.Play.Body, as: PlayBody
  ...
  defp req_timeout, do: Application.get_env(:gateway, :timeout) || 5_000
  ...
  ## Example
      iex> Gateway.Handle.proxies
      [%{port: 8080, to: "http://google.com"}, %{port: 8081, to: "http://example.com"}]
  """
  @spec proxies() :: []
  def proxies, do: Application.get_env :gateway, :proxies, nil

  @doc ~S"""
  Get schemes which is defined as deault.
  ## Example
      iex> Gateway.Handle.schemes
      [:http, :https]
  """
  @spec schemes() :: []
